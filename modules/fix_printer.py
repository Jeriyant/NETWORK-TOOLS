"""Clear Windows print spooler queue (requires Administrator)."""
...
class FixPrinterRunner:
    ...
    def _run(self) -> None:
        try:
            self.on_line("=== FIX PRINTER (Clear Spooler) ===")
            self.on_line("Alur: stop spooler → hapus antrian → start spooler")
            self.on_line("")
            ...
            # Samakan dengan perintah yang diminta
            ...
            self.on_line(r'> del /Q /F /S "%systemroot%\System32\Spool\Printers\*.*"')
            code, out = self._run_cmd(
                [
                    "cmd",
                    "/c",
            ...
            self.on_line("")
            self.on_line("Fix printer (clear spooler) selesai.")
